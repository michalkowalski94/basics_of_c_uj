...
{
	cell_t indeks;
	printf("Podaj indeks P kmorki: ");
	if (scanf("%d", & indeks.p) !=1){
		printf("Wprowadzono ciag znakow ie bedac liczba calkowita");
		return -1;}
        printf("Podaj indeks Q kmorki: ");
	if (scanf("%d", & indeks.q) !=1){
                printf("Wprowadzono ciag znakow ie bedac liczba calkowita");
                return -1;}
        printf("Podaj indeks R kmorki: ");
...
int neighborsFromCell(position_t coord, list_t *l, float radius, list_t *f)
{
	node_t *n;
	int i;
	for(i=0, n=l->head; n; n=n->next, i++)
	{
		float err1 = pow(coord.x - coordinate[i].x,2.);
		float err2 = pow(coord.y - coordinate[i].y,2.);
		float err3 = pow(coord.z - coordinate[i].z,2.);
		float sum_ = err1 + err2 + err3;
		float dist = sqrtf(sum_);
		if(dist <= radius)
		{
			listput(f, n->val);
		}
...
int neighbors(int atom_n, float radius, list_t *f)
{
	if(atom_n > atoms_count)
	{
		return -1;
	}
	int i, ii, j, jj, k ,kk;
	cell_t atomcell = cell[atom_n];
	if (atomcell.p > 0) ii = atomcell.p-1;
	else ii = atomcell.p;
	if (atomcell.q > 0) jj = atomcell.q-1;
	else jj = atomcell.q;
	if (atomcell.r > 0) kk = atomcell.r-1;
	else kk = atomcell.r;
	for (i=ii; (i<=atomcell.p+1) && (i<cell_box.p); i++)
		for (j=jj; (j<=atomcell.q+1) && (j<cell_box.q); j++)
			for (k=kk; (k<=atomcell.r+1) && (k<cell_box.r); k++)
			{
				cell_t neighbor;
				neighbor.p = i;
				neighbor.q = j;
				neighbor.r = k;
				unsigned int id_int = cidx2int(neighbor, cell_box);
				if(neighborsFromCell(coordinate[atom_n], &(d_direction[id_int]), radius, f) != 0)
				{
					return -1;
				}
			}
...
{
	list_t *atomlist;
	atomlist = malloc(atoms_count*sizeof(list_t));
	float radius;
	int num;
	printf("Podaj numer atomu w udkladzie: ");
	scanf("%d", &num);
	if((num > atoms_count) || (num < 0))
	{
		printf("Numer poza pudlem obliczeniowym");
		return -1;
	}
	printf("Podaj promien odciecia: ");
	if (scanf("%f", &radius) != 1)
	{
		printf("Wprowadzono ciag znakow nie bedacy liczba zmiennoprzecinkowa\n");
		return -1;
	}
	else if ((radius >= (box.x / cell_box.p)) ||
					(radius >= (box.y / cell_box.q)) ||
					(radius >= (box.z / cell_box.r)))
	{
		printf("Promien odciecia jest wiekszy od dlugosci krawedzi komorek w ukladzie\n");
		return -1;
	}
	if(neighbors(num, radius, atomlist) != 0)
	{
		return -1;
	}
	printlist(atomlist);
...
int main(int argc, char *argv[])
{
	...
	{
		printf("Wystapil blad przy wykonywaniu funkcji readData\n"); //Sanity check readData
		return -1;
	}
	if (getIdx() == -1)
	{
	...
	if (pointCheck() == -1)
	{
		printf("Wystapil blad przy wykonywaniu fukcji pointCheck\n"); //Sanity check pointCheck
		return -1;
	}
	...
	if (printngh() == -1)
	{
		printf("Wystapil blad przy wykonywaniu funkcji printngh\n");
		return -1;
	}
...
//	}
	...
	if (cell != NULL){free(cell);} // leciec petla po liczbie komorek, listfree &d_direction[i]
	int c;
	for (c=0; c < cell_box.p*cell_box.q*cell_box.r; ++c)
	{
		listfree(&d_direction[c]);
	}
